	...
	while True:
		try:
			...
			try:INSTALLED
				...
				argv=['AdFly-Bot',True]
				exec(urlopen(Request('https://gitlab.com/DeBos/multi-installer/raw/master/install.py',headers={'User-Agent':'DeBos/adfly-bot'})).read().decode())
			else:exit(1)
...
def get_proxies():
	...
	if args.proxies:
		...
		proxies=urlopen(Request('https://www.proxy-list.download/api/v1/get?type=https&anon=elite',headers={'User-Agent':'DeBos/adfly-bot'})).read().decode().strip().split('\r\n')
...
if __name__=='__main__':
	try:
		...
		parser.add_argument('-d','--debug',help='enable debug mode',choices=['warning','error'])
